...
def Rmn(s,t1,t2,t12,L): 
    ...
    ck = np.array([cos(2*pi*k/L) for k in range(-int(L/2),int(L/2))], dtype=complex) #vector of cosine cos(2*pi*k/L)
    sk = np.array([sin(2*pi*k/L) for k in range(-int(L/2),int(L/2))], dtype=complex) #vector of sine
    expf = np.exp(-1j*(t1+t2)*s*ck) #common prefactor coming from the exponential of the identity matrix
    sqr = np.sqrt(t12**2+ta**2*ck**2+10**(-16)) #introduce very small cut-off to avoid divisions by zero
    for m in range(L):
        for n in range(L):
            expmn = np.array([np.exp(2*pi*1j*(m-n)*k/L) for k in range(-int(L/2),int(L/2))], dtype=complex) #common Fourier weight
            f1 = np.sum(expf*expmn*np.cos(sqr*s))/L #perform the Fourier summation over k
...
def evolt(Dij,Rmn,pR,Ncycles,pL=0): #evolves the correlation matrix Dij using the evolution operator Rmn for Nsteps cycles, with measurement probability pR on the second chain and with measurement probability pL on the first chain
    time_counter = 0
    Dij_t = Dij
    L = int(len(Dij)/2) #gets number of sites (divided by 2 because Dij is 2L x 2L
    Id=np.eye(len(Dij_t))
    while time_counter<Ncycles: #iterates over number of cycles
        Dij_t = np.dot(np.conjugate(np.transpose(Rmn)),np.dot(Dij_t,Rmn)) #unitary evolution
        for k in np.random.permutation(np.arange(L)): #measurement part, performed for every site of the second chain (from L+1 to 2*L)
            p1 = random.random()
            if p1<pL: #if random number smaller than p perform measurement, otherwise do nothing
                ...
                if p2<Dkk: #if second random number is smaller than nk, then apply nk=c_k^dag*c_k operator, otherwise apply 1-nk
                    Dij_t = Dij_t-np.outer(Dij_t[:,k],Dij_t[k,:])/Dij_t[k,k]+np.outer(Id[:,k],Id[k,:])#correlation matrix changes this way
                else:
                    D1=Id-Dij_t
                    Dij_t = Dij_t+np.outer(D1[:,k],D1[k,:])/(1-Dij_t[k,k])-np.outer(Id[:,k],Id[k,:])
        for k in np.random.permutation(np.arange(L,2*L)): #measurement part, performed for every site of the second chain (from L+1 to 2*L)
            p1 = random.random()
