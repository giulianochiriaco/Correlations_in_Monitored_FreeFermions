...
def Rmn(s, t1, t2, t12, L):
    ...
    ck = np.array([cos(2*pi*k/L) for k in range(-int(L/2), int(L/2))],
                  dtype=complex)  # vector of cosine cos(2*pi*k/L)
    sk = np.array([sin(2*pi*k/L) for k in range(-int(L/2),
    ...
    for m in range(L):
        for n in range(L):
            # common Fourier weight
            expmn = np.array([np.exp(2*pi*1j*(m-n)*k/L)
                             for k in range(-int(L/2), int(L/2))], dtype=complex)
