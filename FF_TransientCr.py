...
# pass argument from the function call in the bash script
scriptname, L, t1, t2, t12, p2, Nmax, NRseries, multiplier = sys.argv
...
# number of trajectories to simulate in series (one after the other)
NRseries = int(NRseries)
# multiplier for the probability (if it is passed as an integer value using the JOB ARRAY feature)
...
# define folder where to store results. Change it to your path. make sure you actually created the folders
folder = '/g100_work/IscrC_EMEND-Q/Correlations/Cr/t2_' + \
    str(np.round(t2, 2))+'/L'+str(L)
...
for i in range(NRseries):
    ...
    for p1 in p1v:
        ...
        # filename_extension = str(L)+'t1'+str(np.round(t1, 2))+'_t2'+str(np.round(t2, 2))+'_t12'+str(
        #     np.round(t12, 2))+'_p1'+str(np.round(p1, 2))+'_p2'+str(np.round(p2, 2))+'.dat'
        # yet in a more pythonic way
        filename_extension = "{0:1d}t1{1:.2f}_t2{2:.2f}_t12{3:.2f}_p1{4:.2f}_p2{5:.2f}.dat".format(L, t1, t2, t12, p1, p2)

        file11A = folder + '/Cr11/Cr11A_L_'+filename_extension
        file11B = folder + '/Cr11/Cr11B_L_'+filename_extension

        # define file names for the correlations C_22. A different one for every value p1
        file22A = folder + '/Cr22/Cr22A_L_'+filename_extension
        file22B = folder + '/Cr22/Cr22B_L_'+filename_extension

        # define file names for the correlations C_12. A different one for every value p1
        file12A = folder + '/Cr12/Cr12A_L_'+filename_extension
        file12B = folder + '/Cr12/Cr12B_L_'+filename_extension

        try:
            # Run the simulation and extract the arrays that give the correlation functions vs time and r.
            C11A, C12A, C22A, C11B, C12B, C22B, tvec = Transient_Cr_avg(
                L, t1, t2, t12, p2, p1, Nmax, t_step=t_step, R=R)
            # save them to file. Use 'ab+' to append, so that we save all trajectories into one file.
            # the protocol is: 
            #   (i) save the tvec vector as a row (1,Nmax/t_step), so we know the actual times t at which Cr is calculated
